import { handleUploadFile } from "../utils/helperFunctions";
import { getActivityById } from "./supabaseFunctions";
...
// Type that works with both web (File) and native (NativeFile) upload functions
// The platform-specific handleUploadFile will be resolved at bundle time
type UploadableFile = File | NativeFile;
...
export const handleUploadUserAvatar = async (
  ...
  const filePath = await handleUploadFile("avatars", file as any, user);

  const { env } = await import("../env");
  const publicUrl = env.r2DevUrl ? `${env.r2DevUrl}/${filePath}` : filePath;
